<script lang="ts">
	...
	function isStoryViewed() {
		...
		return images.every(
			(_img, idx) => Array.isArray(entry.imagesSeen) && entry.imagesSeen.includes(idx)
		);
	}
...
<div>
	<button type="button" data-sveltekit-prefetch class="cursor-pointer" on:click={() => openStory()}>
		<div
			class="flex h-16 w-16 items-center justify-center justify-self-center rounded-full bg-neutral-200 {isStoryViewed()
				? ''
				: 'bg-gradient-to-t from-[#f09433] via-[#dc2743] to-[#bc1888]'}"
		>
			<img
				src={user.src}
		...
		<h3 class="mt-1 flex items-center justify-center text-sm font-bold text-gray-900">
			<slot />
		</h3>
	</button>
</div>
