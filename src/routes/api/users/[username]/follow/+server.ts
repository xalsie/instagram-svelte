...
export async function POST({ params, locals }) {
    ...
    if (me.username === username) return json({ error: 'You cannot follow yourself' }, { status: 400 });
...
export async function DELETE({ params, locals }) {
    ...
    const me = await requireAuth(locals);
    if (!username || !me) return json({ error: 'Unauthorized' }, { status: 401 });
    if (me.username === username) return json({ error: 'You cannot unfollow yourself' }, { status: 400 });
    const target = await User.findOne({ username });
    if (!target) return json({ error: 'User not found' }, { status: 404 });
